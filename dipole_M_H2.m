function d = dipole_M_H2(Px, Py, Pz, config)
%out = dipole_M_H_phase(Px, Py, Pz)
%This function calculates the dipole moment for ionization of a hydrogen
%atom. d = P ./ (P^2 + 2*Ip).^3
%The function also includes the Coulomb phase, phi_l = Arg[ Gamma( 1 + l + 1i*eta ) ],
%where eta = 1/(k*a_0) is the Coulomb parameter. 
%
%References: Bethe, "QM of One- and Two-Electron Atoms" 1957 pp.
%            Friedrich "Theortical Atomic Physics" 1965 pp. 21
    ...
    %Find Phase
    Arg = 2 - 1i./P; 
    Phase = angle( Gamma( Arg ) );
        
    d = exp(-1i .* Phase) .* abs(Px) ./ (P2 + 2*Ip).^3;

%% Calculate complex gamma function
